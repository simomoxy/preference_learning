...
def show_config_page():
    ...
    if decision_mode == 'oracle':
        ...
        if oracle_type == 'biased':
            ...
            with col1:
                compactness_bias = st.slider(
                    "Compactness Bias",
                    min_value=-1.0,
                    max_value=1.0,
                    value=oracle_config.get('compactness_bias', 0.0),
                    step=0.1,
                    help="Preference for compact shapes"
                )
                config['oracle_config']['compactness_bias'] = compactness_bias
